...
export async function createAndSeedSQLiteDB() {
  try {
    console.log('Creating SQLite database tables...');

    // Create tables manually to ensure compatibility
    await db.run(sql`
    ...
    // Insert admin user
    await db.run(sql`
      INSERT OR IGNORE INTO users (id, email, mobile, first_name, last_name, password, role)
      VALUES ('admin_001', 'admin@salon.com', '9876543210', 'Admin', 'User', ${adminPassword}, 'super_admin')
    `);

    // Insert store
    await db.run(sql`
      INSERT OR IGNORE INTO stores (id, name, address, phone, email)
      VALUES (1, 'VeeNails Premium', 'Bhilai, Chhattisgarh', '9876543210', 'info@veenails.com')
    `);
    ...
    await db.run(sql`
      INSERT OR IGNORE INTO membership_plans (id, store_id, name, description, price, duration_months, discount_percentage, points_multiplier, benefits)
      VALUES (1, 1, 'Gold Member', 'Premium membership', 2500, 12, 15, 2.0, '["15% discount on all services", "Priority booking", "Double points"]')
    `);
    ...
    await db.run(sql`
      ...
      VALUES 
        (1, 1, 'Acrylic Extension', 1800, 120),
        (2, 1, 'Gel Polish', 900, 60),
        (3, 1, 'Nail Art Design', 1500, 90)
    `);

    // Insert products
    await db.run(sql`
      INSERT OR IGNORE INTO products (id, store_id, name, price, cost, barcode, stock, min_stock)
      VALUES 
        (1, 1, 'Essie Base Coat', 450, 280, '123456789011', 45, 5),
        (2, 1, 'OPI Top Coat', 520, 320, '123456789012', 38, 5),
        (3, 1, 'Nail File Set', 280, 150, '123456789013', 120, 10)
    `);

    // Insert login settings
    await db.run(sql`
      INSERT OR IGNORE INTO login_page_settings (id, company_name)
      VALUES (1, 'SalonPro Management System')
    `);
    ...
    console.error('❌ SQLite database creation failed:', error);
    return false;
  }
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  createAndSeedSQLiteDB().catch(console.error);
}